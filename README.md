...
- Постоянная кнопка `☰ Меню` у поля ввода Telegram: в General открывает список MAX-чатов, внутри топика — действия для текущего чата
...
- При создании нового топика автоматически импортируется недавняя история MAX; по умолчанию 20 сообщений, в General → ⚙️ Настройки можно выбрать любое значение 0–100
...
Покрытие: `app/topics.py` (TopicStore), `app/config.py` (загрузка env), `app/max_listener.py` (форматирование, throttle), `app/tg_handler.py` (роутинг команд, поиск и медиа), `app/max_client.py` (опкоды). 222 теста.
